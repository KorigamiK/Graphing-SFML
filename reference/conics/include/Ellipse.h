...
public:
  ...
  static long double CrossMag(const Vector2d &a, const Vector2d &b)
  {
    return a.x() * b.y() - a.y() * b.x();
  }
  static long double CrossMagSqr(const Vector2d &a, const Vector2d &b)
  {
  ...
  // Rotational
  Vector2d VectorAtAngle(long double theta) const
  {
    return v_a * std::cos(theta) + v_b * std::sin(theta);
  }
  Vector2d PointAtAngle(long double theta) const
  {
    return v_a * std::cos(theta) + v_b * std::sin(theta) + v_c;
  }
  Vector2d PointAtAngleH1(long double theta) const
  {
    return v_a * std::cosh(theta) + v_b * std::sinh(theta) + v_c;
  }
  Vector2d PointAtAngleH2(long double theta) const
  {
    return -v_a * std::cosh(theta) + v_b * std::sinh(theta) + v_c;
  }
  Vector2d VelocityAtAngle(long double theta) const
  {
